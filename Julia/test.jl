...
function CheckFeasibility(nsupp, ntypes, nvars, sts, bG_x, bG_t, bh, bA, bb, wq_t, loc, delta, version, x0=nothing, t0=nothing)
...
function SolveOptimization(nsupp, ntypes, nvars, sts, bG_x, bG_t, bh, bA, bb, wq_t, loc, delta, version, x0=nothing, t0=nothing)
    if version == "centralized"
        m = Model(solver=GurobiSolver(MIPGap = 1e-12))
    elseif version == "decentralized"
        m = Model(solver=KnitroSolver(mip_method = KTR_MIP_METHOD_BB, honorbnds=0,
...
function SimulateOptimization(types, fm, loc, deltas, version)
    ...
    for i=1:length(deltas)
        ...
        if version == "decentralized"
            ...
            obj, tra, xs, ts = SolveOptimization(nsupp, ntypes, nvars, sts, bG_x, bG_t, bh, bA, bb, wq_t, loc, delta, version, x_cent, t_cent)
        else
            obj, tra, xs, ts = SolveOptimization(nsupp, ntypes, nvars, sts, bG_x, bG_t, bh, bA, bb, wq_t, loc, delta, version, x_cent, t_cent)
        end
        push!(objs, obj)
...
SimulateOptimization(types, fm, loc, deltas, version)
...
u0 = [0,0,0,1e15,1e15,0,0,0]
v0 = [-33.5,-33.5,-3,-3,-3,-3,-1.5,-1.5]
p0 = [32,32,0,1e15,1e15,0,0,0]
