...
if problem == "decentralized"
    ...
    @variable(m, q[1:nvars], category = :Bin)
    @variable(m, u[1:nvars]>=0)
    @variable(m, w[1:nvars]>=0)
    @variable(m, v[1:nvars])
    @constraint(m, var_rel, x-q .<= 0)
    @NLconstraint(m, var_def[i in 1:nvars], t[i]-x[i]*p[i] == 0)
    @constraint(m, kkt_opt, p - q_x + D*x - u + w + v .== 0)
    @NLconstraint(m, kkt_comp1[i in 1:nvars], x[i]*u[i] == 0)
    @NLconstraint(m, kkt_comp2[i in 1:nvars], (x[i]-q[i])*w[i] == 0)
